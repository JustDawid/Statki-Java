...
class Okret {
    ...
    private final List<Punkt> zajmowanePola;
    ...
    public Okret(int dlugosc){
        ...
        this.trafienia = 0;
        this.zajmowanePola = new ArrayList<>();
    }

    public void dodajPole(int x, int y) {
        zajmowanePola.add(new Punkt(x, y));
    }

    public List<Punkt> getZajmowanePola() { return zajmowanePola; }
...
class Plansza {
    ...
    private void zaznaczPudlaWokolZatopionego(Okret okret) {
        for(Okret.Punkt p : okret.getZajmowanePola()){
...
class PrzeciwnikAI{
    ...
    private Point pierwszeTrafienie = null;
    private Point ostatnieTrafienie = null;
    ...
    public void rozstawFlote(Plansza plansza) {
        int[] dlugoscStatkow = {4, 3, 3, 2, 2, 2, 1, 1, 1, 1};
        for(int dlugosc : dlugoscStatkow){
            boolean postawiony = false;
            while(!postawiony){
                int x = rand.nextInt(plansza.getRozmiar());
                int y = rand.nextInt(plansza.getRozmiar());
    ...
    public String wykonajRuch(Plansza planszaGracza) {
        ...
        if(strzal==null) {
            ...
            testujKierunek = false;
        }

        WynikStrzalu wynik = planszaGracza.strzal(strzal.x, strzal.y);

        if(wynik == WynikStrzalu.JUZ_STRZELANO){return wykonajRuch(planszaGracza);}
    ...
    private boolean czyPoleWazne(int x, int y, Plansza plansza){
        if(x<0 || x>=plansza.getRozmiar() || y<0 || y>=plansza.getRozmiar()) return false;
    ...
    private Point generujLosowyStrzal(Plansza plansza){
        int x, y;
        do {
            x = rand.nextInt(plansza.getRozmiar());
            y = rand.nextInt(plansza.getRozmiar());
...
public class Main extends JFrame {
    ...
    enum StanGry {ROZSTAWIANIE, GRA_TWA, KONIEC_GRY}

    private Plansza planszaGPlansza;
    ...
    private int indeksRozstawieniaStatku = 0;

    private JLabel statusLabel;
    private JLabel orientacjaLabel;
    private JComboBox<String> orientacjaComboBox;
    
    private JTextArea poleHistorii = new JTextArea(); 

    public Main() {
        super("Gra w statki");
        this.planszaGPlansza = new Plansza();
    ...
    private void inicjalizujGUI() {
        ...
        panelGorny.setBackground(new Color(255, 252, 195));
        statusLabel = new JLabel("Rozstaw swoj statek: " + statkiDoRozstawienia[0] + " masztowy. Kliknij na swoją planszę.");
        statusLabel.setFont (new Font("Arial", Font.BOLD, 16));
        orientacjaLabel = new JLabel("Orientacja: ");
        orientacjaComboBox = new JComboBox<>(new String[]{"W poziomie", "W pionie"});

        panelGorny.add(statusLabel);
        panelGorny.add(orientacjaLabel);
        panelGorny.add(orientacjaComboBox);
        ...
        poleHistorii.setBackground(new Color(255, 252, 195));
    ...
    private class GraWidok extends JPanel {
        ...
        public GraWidok() {
            ...
            this.setBackground(new Color(255, 252, 195));
            ...
            this.addMouseListener(new MouseAdapter() {
                ...
                public void mousePressed(MouseEvent e) {
                    ...
                    } else if (aktualnyStan == StanGry.GRA_TWA){
        ...
        private void obsluzKlikniecieRozstawiania(int myszX, int myszY) {
            if(myszX>=startXGracz && myszX<startXGracz+(10*rozmiarKratki) &&
                ...
                int dlugosc = statkiDoRozstawienia[indeksRozstawieniaStatku];
                boolean poziomo = orientacjaComboBox.getSelectedIndex() == 0;

                if (planszaGPlansza.czyMoznaRozmiescic(dlugosc, x, y, poziomo)) {
                    planszaGPlansza.rozmiescStatek(new Okret(dlugosc), x, y, poziomo);
                    indeksRozstawieniaStatku++;

                    if (indeksRozstawieniaStatku < statkiDoRozstawienia.length) {
                        statusLabel.setText("Rozstaw swój statek: " + statkiDoRozstawienia[indeksRozstawieniaStatku]);
                    } else {
                        aktualnyStan = StanGry.GRA_TWA;
                        ...
                        statusLabel.setText("Gra trwa! Wykonaj swój ruch klikając na planszę przeciwnika!");
                        statusLabel.setForeground(new Color(194, 24, 7));
                        dodajLog("=== BITWA ROZPOCZETA ===");
                    }
                    repaint();
                }
            }
        ...
        private void obsluzKlikniecieStrzalu(int myszX, int myszY) {
            if (myszX>=startXPrzeciwnik && myszX<startXPrzeciwnik+(10*rozmiarKratki)
                ...
                if(wynikGracza == WynikStrzalu.JUZ_STRZELANO) return;
                ...
                String logBota = bot.wykonajRuch(planszaGPlansza);
                dodajLog(logBota);

                if(planszaGPlansza.czyWszystkieZatopione()){
        ...
        protected void paintComponent(Graphics g) {
            ...
            rysujPlansze(g, planszaGPlansza, startXGracz, startY, "Twoja plansza", true);
        ...
        private void rysujPlansze(Graphics g, Plansza plansza, int ox, int oy, String tytul, boolean pokazZimneStatki) {
            ...
            for (int i = 0; i < plansza.getRozmiar(); i++) {
                ...
                for (int j = 0; j < plansza.getRozmiar(); j++) {
                    ...
                    g.setColor(new Color(40, 92, 204));
